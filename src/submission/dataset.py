...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ### START CODE HERE
        # -> Picking up the document form self.data
        document = self.data[idx]
        # -> Truncate the document.
        # -> make sure of the upper bound and lower bound
        truncate_index = random.randint(4, min(len(document), int(self.block_size * 3 / 4)))
        document = document[0:truncate_index + 1]
        # Dividing the document into three parts. [prefix] [masked_content] [suffix]
        '''
        As the masked content should be 1/4 of truncated document on an average. 
        declare the upper and lower bound of the masked keeping the 1/4 condition.
        '''
        upper_bound_masked = max(1, len(document) // 5)
        lower_bound_masked = max(1, len(document) // 3)
        # Getting the masked content length
        masked_content_length = random.randint(upper_bound_masked, lower_bound_masked)
        # Calculating the prefix length and suffix length
        prefix_length = random.randint(0, len(document) - masked_content_length)
        #suffix_length = (len(document) - masked_content_length) - prefix_length
        # Getting the prefix, masked_content, suffix
        ...
        # Creating the input and the output of the masked string
        x = masked_string[:-1]
        y = masked_string[1:]
        x = torch.tensor([self.stoi[c] for c in x], dtype=torch.long)
        y = torch.tensor([self.stoi[c] for c in y], dtype=torch.long)
